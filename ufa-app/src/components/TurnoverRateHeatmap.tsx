...
function TurnoverRateHeatmap() {
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = '#ffa500'
    ctx.fillText(`(${throwerPos.x.toFixed(1)}, ${throwerPos.y.toFixed(1)})`, markerCX + 18, markerCY - 4)
    ...
    if (loading) {
      ...
      ctx.font = '12px monospace'
      ctx.textAlign = 'center'
      ctx.fillText(`Loading block predictions... ${Math.round(loadingProgress)}%`, CANVAS_WIDTH / 2, barY + barHeight + 16)
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
      <canvas
        ...
        style={{ cursor: 'crosshair', borderRadius: '8px', maxWidth: '100%', height: 'auto' }}
      ...
      <div style={{ color: '#888', fontSize: '13px' }}>
        Drag the orange dot to move the thrower. Hover to see probability. Click and drag to draw a box and see total probability.
      </div>
    </div>
  )
}

export default TurnoverRateHeatmap
